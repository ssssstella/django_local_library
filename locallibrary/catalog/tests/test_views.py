...
class AuthorCreateViewTest(TestCase):
    ...
    def test_form_date_of_death_initially_has_set_date(self):
        login = self.client.login(username='testuser2', password='2HJ1vRV0Z&3iD')
        response = self.client.get(reverse('author-create'))
        self.assertEqual(response.status_code, 200)

        date_initially_set = '11/11/2023' 
        self.assertEqual(response.context['form'].initial['date_of_death'], date_initially_set)
    
    def test_redirects_to_author_list_on_success(self):
        login = self.client.login(username='testuser2', password='2HJ1vRV0Z&3iD')
        date_proposed = datetime.date(2000, 9, 9) 
        response = self.client.post(reverse('author-create'), {'first_name':'John', 'last_name':'Smith', 'date_of_death': date_proposed})
        self.assertEqual(response.status_code, 302)
